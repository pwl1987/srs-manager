...
function deleteBinding(id) {
  const binding = getBinding(id);
  if (!binding) return null;
  const pushConsumer = db.prepare('SELECT id FROM forward_tasks WHERE source_binding_id = ? LIMIT 1').get(Number(id));
  const recordConsumer = db.prepare('SELECT id FROM record_tasks WHERE source_binding_id = ? LIMIT 1').get(Number(id));
  if (pushConsumer || recordConsumer) throw new Error('Rendition is still referenced by an output');
