  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-[var(--border-soft)] bg-[var(--card)] shadow-[var(--shadow-panel)]">
      <div className="relative aspect-video bg-[oklch(0.22_0.006_258)]">
        ...
        {!playing && <button type="button" onClick={startPreview} disabled={observed?.online !== true || starting} className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-[linear-gradient(180deg,transparent,oklch(0.18_0.006_258/0.46))] text-[var(--foreground)] disabled:cursor-not-allowed">
          <span className={cn('flex h-12 w-12 items-center justify-center rounded-full border backdrop-blur-sm', observed?.online === true ? 'border-[var(--primary)]/35 bg-[var(--primary)]/16 text-[var(--primary)]' : 'border-[var(--border)] bg-[var(--secondary)] text-[var(--text-faint)]')}><Play size={20} fill="currentColor" /></span>
          <span className="text-xs text-[var(--muted-foreground)]">{starting ? t('streams:workspace.console.startingPreview') : observed?.online === true ? t('streams:workspace.console.startPreview') : t('streams:workspace.console.waitingSignal')}</span>
        </button>}
        <div className="pointer-events-none absolute left-3 top-3 flex items-center gap-2">
          <span className={cn('inline-flex items-center gap-1.5 rounded-md border px-2 py-1 text-[10px] font-semibold backdrop-blur-md', observed?.online === true ? 'border-[var(--success)]/25 bg-[var(--success-soft)]/85 text-[var(--success)]' : 'border-[var(--border)] bg-[var(--secondary)]/85 text-[var(--muted-foreground)]')}><Radio size={11} />{observed?.online === true ? 'ON AIR' : 'IDLE'}</span>
