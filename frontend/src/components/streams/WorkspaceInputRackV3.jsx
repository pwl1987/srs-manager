...
function meta(source) {
  ...
  if (source.role === 'PROGRAM' || state === 'ONLINE') return { dot:'bg-[var(--success)]', text:'text-[var(--success)]', label: source.role === 'PROGRAM' ? 'PROGRAM' : state };
  if (['READY','UNKNOWN'].includes(state) && source.role === 'STANDBY') return { dot:'bg-[var(--primary)]', text:'text-[var(--primary)]', label: state };
  if (state === 'OFFLINE') return { dot:'bg-[var(--text-faint)]', text:'text-[var(--muted-foreground)]', label: state };
...
export default function WorkspaceInputRackV3({ workspace, previewingSourceId, onPreview, onManage }) {
  ...
  const groups=[
    ...
    ['STANDBY',normalized.filter(x => x.role === 'STANDBY')],
    ['CANDIDATE',normalized.filter(x => x.role === 'CANDIDATE')],
    ['OFFLINE',normalized.filter(x => x.role === 'OFFLINE' || x.availability === 'OFFLINE')]
  ];
  const standby=normalized.filter(x => x.role === 'STANDBY' && x.availability !== 'OFFLINE').length;
  ...
  return <section className="flex h-full min-h-0 flex-col border-r border-[var(--border-soft)] bg-[var(--card)]/72">
    <div className="flex items-start justify-between gap-2 border-b border-[var(--border-soft)] p-3">
      <div><div className="text-[9px] font-semibold tracking-[.14em] text-[var(--text-faint)]">INPUT RACK</div><div className="mt-1 flex items-center gap-1.5 text-[10px] text-[var(--muted-foreground)]"><ShieldCheck size={11} className={standby ? 'text-[var(--success)]' : 'text-[var(--warning)]'}/>{standby ? standby + ' Standby Ready' : 'No verified standby'}</div></div>
      <button className={btnSecondary} onClick={onManage}><Plus size={12}/>来源</button>
    </div>
    <div className="min-h-0 flex-1 overflow-y-auto p-3">{groups.map(([name,items]) => items.length ? <div key={name} className="mb-3 last:mb-0"><div className="mb-1.5 text-[8px] font-semibold tracking-[.14em] text-[var(--text-faint)]">{name}</div><div className="space-y-1.5">{items.map(source => <SourceRow key={source.id} source={source} previewing={previewingSourceId===source.id} onPreview={onPreview}/>)}</div></div> : null)}
