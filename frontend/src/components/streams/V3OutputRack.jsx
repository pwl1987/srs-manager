...
const MODE_LABELS = { PUSH: '主动推送给第三方', SERVE: '提供地址给第三方拉取', RECORD: '本地录制' };
const STATUS_LABELS = {
  FAILED: '启动失败',
  STALLED: '已停滞',
  RETRYING: '重试中',
  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-black/45" onClick={onClose}>
    <aside className="h-full w-full max-w-[620px] overflow-y-auto border-l border-[var(--border)] bg-[var(--card)] shadow-2xl" onClick={e => e.stopPropagation()}>
      ...
      <div className="space-y-5 p-5">
        ...
        {builderMode === 'pro' && <div className="grid grid-cols-2 gap-3"><div><label className={labelClass}>输出方向</label><select className={inputClass} value={mode} onChange={e => { setMode(e.target.value); if (e.target.value === 'RECORD') setProtection('storage-policy'); }}><option value="PUSH">主动推送给第三方</option><option value="SERVE">提供地址给第三方拉取</option><option value="RECORD">本地录制</option></select></div><div><label className={labelClass}>{mode === 'RECORD' ? '文件格式' : '传输协议'}</label>{mode === 'RECORD' ? <select className={inputClass} value={recordFormat} onChange={e => setRecordFormat(e.target.value)}><option value="mp4">MP4</option><option value="ts">TS</option><option value="audio">AAC 音频</option></select> : <select className={inputClass} value={transport} onChange={e => setTransport(e.target.value)}>{(mode === 'PUSH' ? ['rtmp','rtmps','srt'] : ['rtmp','http-flv','hls']).map(x => <option key={x}>{x}</option>)}</select>}</div></div>}
        {(builderMode === 'pro' || (scene?.mode === 'PUSH')) && mode !== 'RECORD' && <div><label className={labelClass}>访问保护</label><select className={inputClass} value={protection} onChange={e => setProtection(e.target.value)}>{(builderMode === 'pro' ? protectionOptions : (scene?.mode === 'PUSH' ? ['none','url-credential','token','timestamp-signature','provider-credential','passphrase'] : ['none'])).map(x => <option key={x}>{x}</option>)}</select></div>}
        {builderMode === 'pro' && compatibility && <div className={`rounded-xl border p-3 text-[10px] leading-5 ${compatibility.valid ? 'border-[var(--success)]/18 bg-[var(--success)]/5 text-[var(--success)]' : 'border-[var(--destructive)]/18 bg-[var(--destructive)]/5 text-[var(--destructive)]'}`}><div className="font-semibold">{compatibility.valid ? '✓ 当前组合可执行' : '× 当前组合不可执行'}</div><div className="mt-1 opacity-80">{compatibility.message || compatibility.reason_code || '正在校验当前组合'}</div>{compatibility.warnings?.map(w => <div key={w.code} className="mt-1 text-[var(--warning)]">! {w.message}</div>)}</div>}
        {(['PUSH','RECORD'].includes(mode) || ['PUSH','RECORD'].includes(scene?.mode)) && <><div><label className={labelClass}>媒体处理</label><div className="grid grid-cols-2 gap-2"><button className={processing === 'PASSTHROUGH' ? btnPrimary : btnSecondary} onClick={() => setProcessing('PASSTHROUGH')}>节目原始码流</button><button className={processing === 'RENDITION' ? btnPrimary : btnSecondary} onClick={() => { setProcessing('RENDITION'); ensureTemplates(); }}>共享媒体规格</button></div></div>{processing === 'RENDITION' && <div><label className={labelClass}>媒体规格</label><select className={inputClass} value={templateId} onChange={e => setTemplateId(e.target.value)}><option value="">选择媒体规格</option>{templates.filter(x => x.enabled).map(x => <option key={x.id} value={x.id}>{x.name}</option>)}</select><p className="mt-1.5 text-[10px] text-[var(--muted-foreground)]">相同实际编码参数会自动复用同一条共享媒体规格，不重复编码。</p></div>}{(mode === 'PUSH' || scene?.mode === 'PUSH') && <div><label className={labelClass}>目标地址</label><input className={inputClass} value={targetUrl} onChange={e => setTargetUrl(e.target.value)} placeholder={transport === 'srt' ? 'srt://host:port?...' : 'rtmp://...'} /></div>}{(mode === 'RECORD' || scene?.mode === 'RECORD') && <div className="grid grid-cols-2 gap-3"><div><label className={labelClass}>格式</label><select className={inputClass} value={recordFormat} onChange={e => setRecordFormat(e.target.value)}><option value="mp4">MP4（安全分段后整理）</option><option value="ts">TS</option><option value="audio">AAC 音频</option></select></div><div><label className={labelClass}>分段秒数</label><input className={inputClass} type="number" min="2" max="3600" value={segmentSeconds} onChange={e => setSegmentSeconds(e.target.value)} /></div><div className="col-span-2"><label className={labelClass}>存储子目录（可选）</label><input className={inputClass} value={recordSubdir} onChange={e => setRecordSubdir(e.target.value)} placeholder="例如 news/2026-09" /></div></div>}</>}
        <div className="rounded-xl border border-[var(--primary)]/15 bg-[var(--primary)]/5 p-3 text-[10px] leading-5 text-[var(--muted-foreground)]"><Layers3 size={13} className="mr-1 inline text-[var(--primary)]"/>创建动作只保存配置，不会自动启动。点击“启动”后，页面会依次显示计划状态、运行状态和实际观测结果。</div>
      </div>
...
export default function V3OutputRack({ roomId, workspace, scenes = [], onChanged, embedded = false }) {
  ...
  return <section className={embedded ? "h-full bg-transparent p-3" : "rounded-2xl border border-[var(--border-soft)] bg-[var(--card)] p-4 shadow-[var(--shadow-panel)]"}>
    ...
    <div className="space-y-2">{outputs.length ? outputs.map(output => <OutputRow key={output.id} output={output} renditionMap={renditionMap} busy={busyId === output.id} onToggle={toggle}/>) : <div className="rounded-xl border border-dashed border-[var(--border)] px-4 py-8 text-center text-xs text-[var(--muted-foreground)]">还没有配置输出。使用“新建输出”从场景开始。</div>}</div>
