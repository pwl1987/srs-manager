...
for cmd in curl python3 systemctl install ss flock; do need "$cmd"; done
exec 9>/run/lock/srs-manager-origin-hardening.lock
flock -n 9 || fail "another origin-hardening run is already active"
...
if [[ "${CHECK_ONLY:-0}" == "1" ]]; then
  echo "origin-hardening preflight PASS"
  echo "srs_conf=$SRS_CONF"
  echo "manager_env=$MANAGER_ENV"
  exit 0
fi
